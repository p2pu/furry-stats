...
#@cache.cache('discourse.getTopic', expire=86400)
def getTopic(topic_id):
    url = 'http://community.p2pu.org/t/{0}.json'.format(topic_id)
    resp = get(url)
    return resp.json()


#@cache.cache('discourse.getPost', expire='never')
def getPost(post_id):
    resp = get('http://community.p2pu.org/posts/{0}.json'.format(post_id))
    return resp.json()


def getAllTopics():
    url = 'http://community.p2pu.org/latest.json';
    resp = get(url)
    topics = resp.json()['topic_list']['topics']
    while 'more_topics_url' in resp.json()['topic_list'].keys():
        url = 'http://community.p2pu.org' + resp.json()['topic_list']['more_topics_url']
        print("Getting more topics from " + url)
        resp = get(url)
        topics += resp.json()['topic_list']['topics']
    return topics
